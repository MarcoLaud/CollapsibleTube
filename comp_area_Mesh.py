...
analysis = "Angular"
elements = ["32_el", "64_el", "128_el"]

#analysis = "Radial"
#elements = ["1_el", "2_el", "4_el"]

#analysis = "Longitudinal"
#elements = ["10_el", "50_el", "100_el"]

plot_style = ["--", ":", "-."]


fig = figure(figsize=(8, 5))
# title('Mesh sensitivity analysis - {} elements'.format(analysis))
...
for ps, case in enumerate(elements):

    pathlist = Path('./{}/{}/'.format(analysis, case)).glob('rtz*.csv')
    ...
    pp = genfromtxt("./{}/pressure.csv".format(analysis), delimiter=",", skip_header=1)
    pp = pp[:, 1] * (-1)
    ...
    ll = case.split("_")[0]
    plot((areas)/(areas[0]), pp, label=ll+" Elements", linestyle=plot_style[ps],
         linewidth=2)


legend()

fig.savefig("mesh_long.svg", format="svg", dpi=1200)
